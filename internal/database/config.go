...
// NewConfig creates a database configuration based on the environment
// variables required. If any env variables are not set or are invalid then
// this method will throw an error.
func LoadConfig() (Config, error) {
	username, ok := os.LookupEnv("POSTGRES_USER")
	if !ok {
		return Config{}, fmt.Errorf("no POSTGRES_USER env variable set")
	}

	password, err := loadPassword()
	if err != nil {
		return Config{}, fmt.Errorf("loading password: %w", err)
	}

	host, ok := os.LookupEnv("POSTGRES_HOST")
	if !ok {
		return Config{}, fmt.Errorf("no POSTGRES_HOST env variable set")
	}

	portStr, ok := os.LookupEnv("POSTGRES_PORT")
	if !ok {
		return Config{}, fmt.Errorf("no POSTGRES_PORT env variable set")
	}

	port, err := strconv.Atoi(portStr)
	if err != nil {
		return Config{}, fmt.Errorf("failed to convert port to int: %w", err)
	}

	dbname, ok := os.LookupEnv("POSTGRES_DB")
	if !ok {
		return Config{}, fmt.Errorf("no POSTGRES_DATABASE env variable set")
	}

	sslmode, ok := os.LookupEnv("POSTGRES_SSLMODE")
	if !ok {
		return Config{}, fmt.Errorf("no SSLMode env variable set")
	}
